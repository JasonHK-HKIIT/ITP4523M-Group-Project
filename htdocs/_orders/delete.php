...
$statement = $database->prepare("SELECT `oid`, `ostatus`, `pname`, `odeliverdate` FROM `orders` LEFT JOIN `product` ON `orders`.`pid` = `product`.`pid` WHERE `oid` = ? LIMIT 1");
...
if (!empty($order["odeliverdate"]) && (strtotime($order["odeliverdate"]) < strtotime("+2 days")))
{
    http_response_code(400);
    render_error_page("Invalid Request", "The order was no longer deletable due to processing constraints.");
    exit;
}
